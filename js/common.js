...
function repositionDialog(){
	...
	var bottomSize = 30;
	var clientHeight = getViewHeight();
	var bodyHeight = $body.style.height.replace("px","");
	
	var pigPositon = getElemDis($pig);
	var pigHeight = $pig.clientHeight;
	var pigWidth = $pig.clientWidth;
	
	
	if(clientHeight<(pigPositon.bottom+bottomSize) || (bodyHeight*0.2-bottomSize) < (bodyHeight-clientHeight)){
		var bt = bodyHeight-clientHeight+bottomSize;
		$pig.style.bottom = bt+ "px";
		var dialogBt = bt+pigHeight;
		$dialog.style.bottom = dialogBt+"px";
	
	}else{
		var dialogBt = bodyHeight*0.2+pigHeight;
		$dialog.style.bottom = dialogBt+"px";
		$pig.style.bottom = "20%";
	}
	
		var sayBt = (bodyHeight-clientHeight) > 0 ? (bodyHeight-clientHeight) : 0;
		$say.style.bottom = sayBt+"px";
}
